...
def zero_pad(lst):
    ...
    print('Maximum Signal Length for Padding: %d' % pad)

    # compute average signal length
    sum_length = []
    for i in lst:
        sum_length.append(len(i))
    print('Average signal length before padding: %5f' % np.mean(sum_length))
...
def get_patient_data(patient,norm=True, sample_plot=False):
    ...
    return ecg_data.signal, ecg_data.V,ecg_notes
...
def isolate_patient_data(patients,classes,classes_further,classes_reducer=None, min_HR= 40,max_HR= 140,fs=360,verbose=False,plot_figs=True):
    ...
    for i,patient in enumerate(patients):
        ...
        ecg_signal, ecg_V, ecg_notes= get_patient_data(patient)
        peaklist= ecg_notes.sample_num.astype(int).values 
        for c in classes.values():
            class_loc=[]
            if classes_reducer != None:
            ...
            for n in range(1,len(peaklist)-1):
                ...
                if peaklist[n] in set(class_loc):
                    ...
                    peak_data_signal= ecg_signal[peaklist[n]-delta2:peaklist[n]+delta1]
                    peak_data_V = ecg_V[peaklist[n]-delta2:peaklist[n]+delta1]
                    if hr_sample_len(max_HR) <= len(peak_data_signal) <= hr_sample_len(min_HR):
                        isolated_beat_signal.append([patient,get_HR(peaklist,fs=fs),c]+peak_data_signal.tolist())
                        isolated_beat_V.append([patient, get_HR(peaklist, fs=fs), c] + peak_data_V.tolist())
    ...
    print('\nPadding...\n')
    isolated_beats_signal= zero_pad(isolated_beat_signal)
    isolated_beats_V = zero_pad(isolated_beat_V)

    X_signal =isolated_beats_signal[:,3:].astype(float)
    X_V = isolated_beats_V[:,3:].astype(float)

    Y =isolated_beats_signal[:,:3]



    avg_samp=np.array([len(l) for l in isolated_beat_signal]).mean()
    ...
    if plot_figs== True:
        ...
        plt.subplot(121)
        x=[len(elem) for elem in isolated_beat_signal]
    ...
    print('Data Loaded | Shape:{}\n'.format(isolated_beats_signal.shape))
...
def most_common_conditions(patients,top_k):
    ...
    output = Counter(allp).most_common(top_k)
...
#     patients=[101]
#     X_signal, X_V,y,isolated_beat=isolate_patient_data(patients=patients, classes=classes,classes_further=classes_further,
